...
stats_csv_file = 'store/car_stats.csv'
images_folder = 'store/road_images/'

def read_stats(stats_csv_file, images_folder):
    stats_table = read_stats_from_csv(stats_csv_file)
    
    ids = stats_table['id']
    images = read_images_from_fs(images_folder, ids)

    X_table = stats_table[['speed']].copy()
    X_table = X_table.assign(image=images)

    y_table = stats_table[['steerAngle']].copy()

    #X - concat together all columns and image array
    return {"X": X_table.values, "y": y_table.values}
